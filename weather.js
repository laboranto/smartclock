...
const API_KEY = ("22a5f23111bd333c3971dd4f3391381f");
...
function onGeoOk(position){
  ...
  fetch(apiUrl).then(response=> response.json())
    .then(data=> {
      ...
      weather.innerText = '';      weather.insertAdjacentHTML('afterbegin', `${city}, <img src="icons/weather/${currentWeather}.svg" alt="${currentWeatherAlt}"> ${currentTempInt}°C`);
    })
...
function onGeoError(){
  weather.innerText = '';
  weather.insertAdjacentHTML('afterbegin',`<span class="font-grey">날씨 정보를 불러오지 못했습니다</span>`);
}
